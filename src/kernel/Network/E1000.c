...
int e1000_init(void) {
    ...
    e1000_write_reg(REG_CTRL, ctrl | (1 << 6) | (1 << 5)); // SLU + ASDE
    ...
    for (int i = 0; i < 32; i++) {
        ...
        e1000_dev.tx_descs[i].status = 1; // DD bit
...
void e1000_interrupt_handler(void) {
    ...
    // Check RX descriptors even without interrupt
    ...
    while (e1000_dev.rx_descs[idx].status & 1) {
        ...
        PRINT(GREEN, BLACK, "[E1000] RX: ");
        print_unsigned(len, 10);
        PRINT(WHITE, BLACK, " bytes\n");
        
        // Print first 32 bytes for debug
        PRINT(WHITE, BLACK, "[E1000] Data: ");
    ...
    if (got_packets > 0) {
        PRINT(GREEN, BLACK, "[E1000] Processed ");
        print_unsigned(got_packets, 10);
        PRINT(WHITE, BLACK, " packets\n");
    }

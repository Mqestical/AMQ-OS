...
static void mix_columns(uint8_t *state) {
    for (int i = 0; i < 4; i++) {
        ...
        state[i + 12] = gmul(a0, 3) ^ a1 ^ a2 ^ gmul(a3, 2);
    }
}
...
static void inv_mix_columns(uint8_t *state) {
    for (int i = 0; i < 4; i++) {
        ...
        state[i + 12] = gmul(a0, 11) ^ gmul(a1, 13) ^ gmul(a2, 9) ^ gmul(a3, 14);
    }
}
...
static void add_round_key(uint8_t *state, const uint32_t *key) {
    for (int i = 0; i < 4; i++) {
        ...
        state[i + 4] ^= (key[i] >> 16) & 0xFF;
        state[i + 8] ^= (key[i] >> 8) & 0xFF;
        state[i + 12] ^= key[i] & 0xFF;
    }
}
